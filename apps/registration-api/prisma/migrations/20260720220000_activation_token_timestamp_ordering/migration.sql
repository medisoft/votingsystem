ALTER TABLE "ActivationToken"
  ...
  ADD CONSTRAINT "ActivationToken_delivery_timestamp_check" CHECK (
    "deliveredAt" IS NULL OR
    ("deliveredAt" >= "generatedAt" AND "deliveredAt" <= "expiresAt")
  ),
  ADD CONSTRAINT "ActivationToken_lifecycle_check" CHECK (
    ...
    ("status" = 'REVOKED' AND "redeemedAt" IS NULL AND "revokedAt" IS NOT NULL AND
      "revokedAt" >= "generatedAt" AND "revokedAt" <= "expiresAt")
  );
